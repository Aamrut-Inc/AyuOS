...
- `syncWearables()` (`extract/wearables/sync.ts`) used to always request only the last `WEARABLE_SYNC_DAYS` (default 90) days, every call, so the local mirror could never accumulate more than a rolling recent window even though `wearables-db` had full history. Fixed: it now checks `MAX(ts)` already stored for that user in `timeseries.readings` — a brand-new user gets a full-history pull (10-year lookback, effectively unbounded), and every sync after that only requests since the last known reading (with a 1-day overlap buffer). `WEARABLE_SYNC_DAYS` no longer exists.
